...
function surface(mode: string, path: string, preference?: string, embedded = false) {
  ...
  const context = {URL, URLSearchParams, window, localStorage: storage, sessionStorage: storage,
  ...
  return {navigations, values, click: (target: object) => handlers.click({target: {closest: () => target}, preventDefault() {}})};
}
