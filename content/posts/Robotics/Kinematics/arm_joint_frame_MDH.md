...
本文参考 Craig 的《Introduction to Robotics（机器人学导论）》，梳理改进 DH（MDH）坐标系的建立方法、四个参数的定义，以及旋转矩阵、位移向量和齐次变换矩阵之间的关系。

Craig 的 MDH 方法特别适合描述树状或分叉机器人结构。MDH 坐标系固连在对应连杆上，原点通常位于连杆前端的关节位置；标准 DH 与 MDH 的坐标系归属和变换顺序不同，使用时不能混用参数定义。
...
如果两个关节不相邻，则需要逐级连乘相邻坐标系之间的旋转矩阵，得到两个目标坐标系之间的关系。
...
MDH 参数提供相邻连杆变换的结构化描述，旋转矩阵和位移向量给出其几何含义，最终的 $4\times4$ 齐次变换矩阵则用于逐级连乘和工程计算。建模时应始终保持坐标系归属、参数下标和变换方向一致。
