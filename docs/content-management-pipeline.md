...
本博客要求从父级到最深层级逐级列出完整路径。例如文章属于“控制 → 机器人学 → 运动学”：
...
替换该文章的整个 `columns` 数组。例如移动到“控制 → 系统架构 → ROS2”：
...
```powershell
rg -n '控制/机器人学/运动学' content/posts
```

然后逐篇修改 `columns`。当没有任何文章引用某个专栏名称时，该专栏会在下一次构建时自动消失。

重命名多级专栏时，必须同步修改所有后代路径。例如把 `控制/机器人学` 改为 `控制/Robotics`，也要修改：
